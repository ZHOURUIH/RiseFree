...
public class RibbonTrail
{
	...
	protected float mMinDistance = 0.01f;
	...
	public void addSection(Vector3 start, Vector3 end)
	{
		...
		{
			mSectionList.RemoveAt(0);
		}
		// 如果与上一个点的end距离太近,则不添加
		if (mSectionList.Count > 0 && MathUtility.getLength(mSectionList[mSectionList.Count - 1].mEnd - end) < mMinDistance)
		{
			return;
		}
